...
class TblNonCompliantEducation extends Model
{
    ...
    protected $column = ['id', 'region', 'province', 'muni', 'brgy',
        ...
        'year','period','brgy_id'];  
    protected $skipColumn = ['id','hh_id','entry_id','lastname','firstname','middlename','ext','bday','brgy_id','school_id','dom_sch_id','year'];
    ...
    protected function buildQuery(){  
        $this->query = '';  
        $merge = array_merge($this->skipColumn,['brgy','psgc','school_name','school_brgy','dom_sch_name','dom_sch_brgy']);
    ...
    protected function setFilter(){         
        ...
        foreach($this->filters AS $f){                    
            foreach($this->column AS $s){
                if(isset($this->search[$s]) AND $this->search[$s]!='null'){   
                  $tmp = json_decode($f[$s]);
                  foreach($tmp AS $r){	        			        		
                    if(in_array($r,$this->search[$s])){
                      $this->{$s}[$f['year']][] = $r;
                    }
                  }        	
                }
    ...
    public function getPaidUnpaid($year){
        //return DB::table(''.$this->table.$year)->select(DB::raw('paid, COUNT(*) as records, SUM(grand_tot) as amount'))->groupBy(['paid'])->get();
